...
public class OpenIntervalTest {
	...
	public void testIsIntersectedOverlapingByLeftWithEquals() {
		Interval one = new IntervalBuilder().type(IntervalType.OPEN).min(3).max(14).build();
		Interval another = new IntervalBuilder().type(IntervalType.OPEN).min(3).max(7).build();
		assertTrue(one.isIntersected(another));
	}
	...
	public void testIsIntersectedNotOverlapingByLeftSameValue() {
		Interval one = new IntervalBuilder().type(IntervalType.OPEN).min(3).max(14).build();
		Interval another = new IntervalBuilder().type(IntervalType.OPEN).min(14).max(18).build();
		assertFalse(one.isIntersected(another));
	}
